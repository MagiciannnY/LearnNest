<template>
  <div class="w-screen h-screen bg-gray-50 text-gray-800 font-sans overflow-hidden">
    <header class="fixed top-0 left-0 right-0 h-12 bg-white border-b border-gray-200 flex items-center px-4 z-50 shadow-sm">
      <button
        ...
        class="w-8 h-8 bg-transparent border-0 rounded-lg flex items-center justify-center hover:bg-gray-100 transition-colors duration-200"
        title="切换菜单"
      >
      ...
      <button
        @click="startNewChat"
        class="ml-2 w-8 h-8 bg-transparent border-0 rounded-lg flex items-center justify-center hover:bg-gray-100 transition-colors duration-200"
        title="新的聊天"
      >
    ...
    <div class="flex h-full w-full overflow-hidden pt-12">
      ...
      <!-- 左侧菜单栏 -->
      <aside
        class="relative bg-gray-100 shadow-md border-r border-gray-200 overflow-y-auto transition-all ease-in-out duration-500"
        :style="{ width: isCollapsed ? '0px' : '16.666667%', minWidth: isCollapsed ? '0px' : '200px' }"
      >
        <ul class="space-y-1 text-sm pt-4 flex flex-col items-center">
          <li>
            <button
        ...
        <div class="w-full px-2 pt-4">
          <p class="text-xs text-gray-500 mb-2 pl-4" v-if="!isCollapsed">历史记录</p>
          <ul class="space-y-1 text-sm flex flex-col items-center">
      ...
      <main class="flex-1 flex flex-col justify-between bg-white overflow-hidden">
        <div class="flex-1 overflow-y-auto p-6">
          <Wiki v-if="showWiki" />
          <AskForm
            v-else-if="showAskForm === 1 || showAskForm === 2"
  ...
  <teleport to="body">
    <div
      v-if="showUploaderModal"
      class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50"
      @click.self="showUploaderModal = false"
    >
    <div class="bg-white p-8 rounded-2xl shadow-xl w-[640px] max-h-[90vh] overflow-auto">
      <FileUploader />
    </div>
    </div>
  </teleport>
...
const showUploaderModal = ref(false)
const showWiki = ref(false)
const showAskForm = ref(1)
...
onMounted(async () => {
  try {
    const res = await axios.get(`http://localhost:8002/history/load/${userId}`)
...
async function loadSession(sessionId) {
  try {
    const res = await axios.get(`http://localhost:8002/history/session/${sessionId}`)
    const content = res.data.content
    const title = res.data.title || '历史会话'
    ...
    currentSessionHistory.value = qaPairs
    showAskForm.value = 2
    showWiki.value = false
  } catch (error) {
